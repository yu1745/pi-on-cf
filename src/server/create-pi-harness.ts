...
const BASE_SYSTEM_PROMPT = [
  ...
  'In the bash tool there is no native binary execution (no npm, python, node). For searching, piping, or any text processing use bash (grep, sed, awk, jq, find, sort, uniq, wc, etc. all work, with pipes and redirections) — it is strictly more capable than a dedicated search tool and operates on the same filesystem as read/write. `curl` is available inside bash for fetching web content (gated by the Worker\'s outbound policy), useful when you want to pipe a response into grep/jq/sed; for structured HTTP requests use the fetch tool. For git use `git clone`/`git ls-remote` inside bash.',
