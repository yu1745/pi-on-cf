...
const BASE_SYSTEM_PROMPT = [
  ...
  'There is no network and no native binaries in the bash tool; use the file tools, the git tool, and the fetch tool for everything else. For searching, piping, or any text processing use the bash tool (grep, sed, awk, jq, find, sort, uniq, wc, etc. all work, with pipes and redirections) — it is strictly more capable than a dedicated search tool and operates on the same filesystem as read/write.',
