...
export function createWorkspaceTools(workspace: MemoryWorkspace, options?: CreateWorkspaceToolsOptions) {
  ...
  const bashTool: AgentHarnessTool<undefined, typeof bashSchema> = {
    ...
    description:
      'Run a bash command against the in-memory workspace. Supports the full core unix toolset — pipes (|), redirections (> >> 2> &>), command chaining (&& || ;), variables, globs, loops, and functions — plus text tools like grep, sed, awk, jq, sort, uniq, wc, head/tail, cut, tr. `curl` is the HTTP surface (e.g. `curl -s URL | jq .field`). `git clone` and `git ls-remote` are also available inside bash (SSH/bare URLs are rewritten to HTTPS automatically; GitHub clones stream a codeload tarball with no .git to stay within memory limits). `git clone` for GitHub repos accepts non-standard `--include <glob>` / `--exclude <glob>` flags (repeatable; `**` spans `/`, `*` within a segment) to keep only the paths you need — use them when cloning a large repo and you only need a subtree, to cut the in-memory footprint. Filesystem is shared with the read/write/edit tools: a file written with `write` is visible to `cat`, and vice versa. cwd is /workspace. There is no native binary execution (no npm, python, node).',
